...
def new_chat(subject):
    ...
    # Force full page reload for HTMX requests to reinitialize JavaScript
    if request.headers.get('HX-Request') == 'true':
        response = make_response('')
        response.headers['HX-Redirect'] = url_for('min.chat', room_id=chat.room_id)
        return response
...
def chat(room_id):
    ...
    # If HTMX request, force a full page reload to reinitialize JavaScript
    if request.headers.get('HX-Request') == 'true':
        response = make_response('')
        response.headers['HX-Redirect'] = url_for('min.chat', room_id=room_id)
        return response
